...
- **`prototype-v2/` implements that shape**: a strict-TypeScript Home Memory console (Home / Ask / Capture / Setup / Spaces / Belongings / Operations / Review / Plan / Commit Ledger) over an event-sourced Place Graph. Two engineering efforts on `codex/full-stack-low-latency` are complete and reviewed: the **loss-safe low-latency runtime** hardening (ADR-0003 — command-as-transaction, atomic file persistence, private query indexes, bounded HTTP/agent transport, lazy Three.js, zero settled RAF) and a **2D/3D spatial-review polish** pass (read-only projection per ADR-0002 — state-first selection with 2D↔3D parity, hover + fitted selection outline, layer + X-ray toggles, locate↔inspect linkage, lighting/token elegance, motion + a11y). Both closed with two-axis `code-review`. Durability is deliberately a single-Store / single-writer prototype guarantee; independent browser tabs are not coordinated by an atomic cross-tab CAS. Run `cd prototype-v2 && npm install && npm run verify` (currently all green); use `npm run benchmark` and `npm run browser-benchmark` for explicitly separated runtime and browser distributions. Current generated reports live in `prototype-v2/renders/`.
...
- **The repository follows the current `mattpocock/skills` flow** (`AGENTS.md` §Development workflow, aligned to the latest `ask-matt` router). The multi-session V1.x effort is a `wayfinder` map at `.scratch/room-recall-v1x/map.md`. Decisions **01–03 are now resolved**: (01) moving is the *acquisition/activation event*, not the durable promise — the durable promise is **Ownership Recall + pre-purchase recall**, North Star = *Monthly Trusted Recall Outcomes*; (02) the **Container Snapshot fast lane** is the five-minute activation path (runnable prototype in `.scratch/room-recall-v1x/activation-prototype/`); (03) an evidence-proxy (desk research + synthetic personas — **not** field evidence) returned **REVISE**: advance the on-ramp to a *real* ≥5-user field test with a revised protocol, but do not proceed to a V1.x `spec.md` on the proxy. Downstream reconstruction tickets (04+) stay blocked on real evidence. No V1.x `spec.md` is published until the map is clear.
- **Docs & tracker**: `docs/` holds the live product documents; retired material is in `docs/archive/` (**nothing there is current**). New tracker state lives privately under `.scratch/` (excluded via `.git/info/exclude`); existing tracked `.scratch/` files remain historical repository content. A GitHub remote exists for source control, but GitHub Issues is not the tracker. See `AGENTS.md` and `docs/agents/`.
