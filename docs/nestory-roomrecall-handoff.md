...
> 3. **V2 prototype built (strict TypeScript, now v2.5)**: `prototype-v2/` is the Home Memory Operations Console (Home / Ask / Capture / Setup / Spaces / Belongings / Operations / Review / Plan / Commit Ledger) over an event-sourced Place Graph store. The August 11 pass completes a system-wide UI/runtime hardening: recomposed responsive UI, fixed accessible household-scale pages, controlled drafts/focus/toasts, lazy and reduced-motion-aware Three.js, loss-safe single-writer Store transactions/import, cold-query indexing, atomic file persistence, bounded Agent/HTTP execution, optional same-process idempotency, and truthful seed/household performance reports. Independent browser tabs are not an atomic multi-writer system. The room scan remains an explicitly labeled interaction prototype; only accepted observations enter the Review pipeline. Run `npm run verify`, `npm run benchmark`, and `npm run browser-benchmark`; generated evidence is in `prototype-v2/renders/`.
...
- `prototype-v2/`: the **current** V1 console, in strict TypeScript. Event-sourced Place Graph, moving + kits + Review + Commit Ledger + agent/sync/eval surfaces, now with redesigned Home, unified Capture, product dimensions, and shared-coordinate 2D/3D Plan (`src/spatial.ts`). The current assertion count and browser status are generated by `npm run verify` in `prototype-v2/renders/verification-report.md`; screenshots include `nestory-home.png`, `nestory-plan-3d.png`, `nestory-capture-scan.png`, boot recovery, and mobile coverage.
...
State persists in localStorage; use Commit Ledger -> Reset to seed to start over. Keep only one tab actively editing a given home because this prototype has no cross-tab atomic compare-and-swap. `window.nestory` exposes the store and view hooks for scripting (typed as `NestoryHooks` in `src/app.ts`).
...
- `prototype-v2/src/app.ts` + `index.html` + `nestory.css`
...
- `prototype-v2/src/server.ts`
  - The local sync service (P0.10): atomic file-backed `StorageLike` adapter + zero-dependency HTTP API. Read views mirror the UI; `/tools`, `/ask`, and import dispatch through the same Store/toolkit boundary; decision tools 403 without `{ "confirmed": true }`; optional idempotency is bounded to one process and Store object; export/import remain schema-compatible.
...
- `prototype-v2/src/benchmark.ts` + `src/browser-benchmark.ts`
  - Repeatable seed/household runtime and browser measurements. Warm repeated HTTP, distinct cold HTTP, DOM/layout, raw RAF, FCP, app-ready, and compositor-surface latency are labeled separately; only the declared metrics are budgeted.
...
- Keep localStorage as the offline fallback for one active writer. Multiple tabs or devices require explicit conflict detection/coordination; do not silently treat them as safe last-write-wins writers.
